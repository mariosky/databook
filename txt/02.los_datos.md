...
En el software Orange para cada atributo indicaríamos sus metadatos de la siguiente manera:
...
``` 
Es importante conocer el tipo y rol de los atributos, por ejemplo en este caso vemos que todas las características son continuas y la etiqueta es categórica. Entonces tenemos que utilizar algoritmos que puedan recibir este tipo de datos. Existen algoritmos por ejemplo, que no pueden trabajar  datos categóricos o aquellos en los que la etiqueta debe ser binaria. En la sección de preprocesamiento veremos varias técnicas que podremos utilizar para modificar los datos de tal manera que podamos emplear de mejor manera la mayor cantidad de algoritmos posible.   

### Matrices

El uso de matrices nos permite representar relaciones entre objetos o atributos. 

#### Documento-Termino 
Esta matriz describe a una colección de documentos por medio de la frecuencia de las palabras o términos que aparecen en ellos. Cada renglón representa un documento y cada columna corresponde a un termino. Los términos se extraen inicialmente de todos los documentos. En cada celda se tiene algún valor representativo de la relación documento-termino. Una representación común es el peso Tf-idf (del inglés Term frequency – Inverse document frequency) descrito en la sección de preprocesamiento. A manera de ejemplo consideremos los siguientes dos documentos:

``` 
D1 : "La vida no vale nada, no vale nada la vida"
D2 : "Toda la vida coleccionado mil amores" 
```
Podemos representar a estos documentos con la siguiente matriz, la cual captura la frecuencia de un termino en los documentos:
...
En este ejemplo básico podemos ver el resultado de un proceso para pasar de la colección de documentos a la matriz. El proceso incluye el remover algunos términos ya que son demasiado comunes de tal manera que no nos dicen nada distintivo del documento. Una característica importante de la matriz es que la mayoría de los elementos tendrán el valor de cero, por esto se dice que es una matriz escasa. Las matrices escasas normalmente se implementan utilizando estructuras de datos especiales para no desperdiciar espacio y que el procesamiento sea más rápido.  
