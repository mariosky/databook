...
Novatos y novatas, el porqué están aquí no me importa. Ninguno es un santo. Ya veo en sus caras la pregunta ¿valdrá la pena?. Ser detective solo se trata de una cosa: saber elegir. Así que empecemos. Aquellos que crean que todo se resuelve con solo saber a quién golpear o a quién pagar. No pierdan su tiempo, ahí está la puerta, ustedes ya lo resolvieron. El resto, esos que creen que los casos se resuelven reuniendo evidencias, investigando la escena del crimen y usando la computadora. De una vez les digo, la realidad no es como en CSI o en su serie favorita. No tenemos cámaras mágicas o el clásico nerd que con solo golpear cien veces el teclado ya tiene una respuesta. Ustedes tendrán que aprender a pensar y ensuciarse como mineros. Principalmente a ensuciarse.

Vamos a entrenarlos primero en Minería de Datos, pero solo aquellos que pasen las pruebas podrán decir que son detectives de datos. Empezamos.  
...
**Atributos:** Son las propiedades o características que tienen los objetos de cierto tipo. Todas las observaciones que se hagan del mismo tipo de entidad tendrán los mismos atributos. Para el ejemplo anterior los atributos serán `{id, nombre, estatura, temperatura, presión diastólica, presión sistólica, frecuencia cardíaca }`.

**Valores:** El estado de un objeto corresponde a los valores que tienen sus   atributos en un momento dado. Los valores a su vez son objetos. Para analizar las observaciones estadísticamente para utilizarlas como pruebas debemos de considerar el tipo de dato:

* Dato *cualitativo*: Normalmente la observación es expresada en palabras. Por ejemplo, la descripción de una escena del crimen. Puede etiquetar al objeto como miembro de una clase o grupo. En el caso del atributo  los valores podrían ser:  mexicana, sushi, italiana, etc.
