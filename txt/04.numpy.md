...
Ahora vamos a modificar a aquellos elementos que cumplan con la condición:
...
Los ciclos se hacen a partir del primer eje:
...
Podemos leer todos los elementos del arreglo secuencialmente:
...
En Python utilizamos la función *range()* para generar listas de números enteros secuenciales. De manera similar en NumPy utilizamos *arange()* para generar arreglos de NumPy con valores secuenciales.
...
array([ 0.  ,  0.33,  0.66,  0.99,  1.32,  1.65,  1.98,  2.31,  2.64,
        ...
        8.91,  9.24,  9.57,  9.9 ])
```
En el segundo caso utilizamos un flotante para decirle a la función que nos genere números del 0 al 10, avanzando con pasos de 0.33. Un problema que podemos tener en este caso es que no sabemos cuantos elemento se van a generar y que tanto se acercará al valor final, en este caso 10. Para evitar esto, tenemos la función *linspace()* a la cual le decimos, dáme 31 valores entre 0 y 10 inclusive:
...
```
De esta manera sabemos exactamente cuantos valores tendremos. Esta función se utiliza mucha para graficar funciones, ya que nos genera los valores de *x* que vamos a evaluar en la función.  
...
Las funciones generan arreglos de dimensión-1, podemos crear arreglos de mayor rango con la fucnión de *reshape()*:
``` python
