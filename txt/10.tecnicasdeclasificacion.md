...
Una vez definida la función objetivo podemos evaluar que tan bueno es el vector
de pesos $\vec{w}$ y podemos ajustarlo utilizando alguna técnica optimización.
Decimos aquí que el perceptron va a aprender los pesos, a partir de el
entrenamiento que consiste en clasificar los objetos, si no se comete un error,
pues no ajustamos nada, pero si se comete un error de clasificación debemos
ajustar los pesos. Vamos a utilizar la técnica del gradiente descendiente
estocástico para ir ajustando los pesos de manera iterativa. Esto es posible ya
que la función del error $E_P(\vec{w})$ es lineal y diferenciable. La actualización de los pesos está dad por:

$$
\vec{w}^(\tau+1)=\vec{w}^(\tau)-\eta \nabla E_P(\vec{w})= \vec{w}^(\tau)+\nabla\phi(x_n)t_n
$$
