...
Rusell y Norvig [-@russell2016artificial] el término inteligencia artificial se
aplica cuando una máquina imita las funciones cognitivas que nosotros los
humanos asociamos con otras mentes humanas, como por ejemplo: **aprender** y
**resolver problemas**. Precisamente, una de las primeras aplicaciones de la
inteligencia artificial han sido los [Sistemas Expertos][1], los cuales pueden
emular la toma de decisiones de un humano experto, tal como
un médico. Un exponente importante de este tipo de sistemas
fue  [Mycin][2] el cual se basaba en un sencillo motor de inferencia, que
manejaba una base de conocimiento de aproximadamente unas 500 reglas. Estas
reglas [representaban el conocimiento][3] de
...
[elicitación del conocimiento][4] para después [representar dicho
conocimiento](https://es.wikipedia.org/wiki/Representaci%C3%B3n\_del\_conocimiento)
de alguna manera para después almacenarlo en una \*base
...
para descubrir patrones que les ayuden a tomar decisiones. Por ejemplo, en la
película "The Big Short" podemos ver al inversionista Michael
Burry analizando grandes cantidades de datos, para después predecir el eminente
desplome de la burbuja inmobiliaria. Como podemos imaginar, este proceso no es
trivial, no es simplemente hacer una consulta en una base de datos o en un motor
de búsqueda. Es necesario encontrar patrones que involucran diferentes variables
y relaciones no lineales entre ellas. El KDD sigue evolucionando involucrando
cada vez más áreas de investigación: aprendizaje automático, reconocimiento de
patrones, computación inteligente, estadística, procesamiento de lenguaje
natural, visualización, ingeniería de software entre otras.
...
![El proceso del KDD según Brachman y Anand][image-2]
...
4. Dependiendo de los objetivos se deben *transformar* los datos para
   ...
   transformarlo en vector representativo. Una imagen puede ser transformada en
...
5. Este paso consiste en *seleccionar la tarea de minería de datos* adecuado de
...
usual actualmente. Algo muy importante es que los modelos deben de representar
también a datos nuevos. Digamos nuevos clientes que se subscriban el mes
siguiente.  
...
mayoría de los proyectos los otros pasos requieren de mayor trabajo. La minería
de datos entonces podemos definirla como: El proceso de búsqueda de patrones
...
predicciones a partir de los datos. Podríamos decir que en el proceso de vamos a
...
partir de los datos. La ciencia apoyada en datos (en inglés data-driven science)
al igual que el KDD es una campo interdisciplinario que incluye métodos
científicos, procesos, y sistemas para extraer conocimiento o entendimiento a
partir de los datos.
...
datos. Normalmente los datos se almacenan en un servidor central. En el caso de
...
algoritmos tradicionales resultan inadecuados. El usar Big Data no implica
Minería de Datos, muchos sistemas solamente requieren procesar los datos, por
...
![Reconocimiento de patrones en multimedia][image-5]\
...
*sue*, etc. El problema de Clasificación es el de identificar a que clase
...
* **Alta Dimensión.** Hace algunos años era inconcebible tener objetos con
...
![El tamaño del espacio aumenta exponencialmente haciendo que los datos disponibles se vuelvan dispersos][image-6]
